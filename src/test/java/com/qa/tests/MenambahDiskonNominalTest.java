...
public class MenambahDiskonNominalTest extends BaseTest{
...
//		introPage = new IntroScreenPage();
	...
	@Test(priority = 2)
	public void tambahDiskonPersen() {
		penjualanPage.pilihKategori1();
		penjualanPage.TambahKeranjang1();
		itemPopUp = penjualanPage.PilihKeranjang1();
		itemPopUp.selectDiskonHarga();
		itemPopUp.enterDiskonPersen(getStrings().get("discount_price"));
		penjualanPage = itemPopUp.pressSaveBtn();
		
		String expectedDsikon = getStrings().get("expected_diskon_nominal");
		String actualDiskon = penjualanPage.getDiskonNominal();
		Assert.assertEquals(actualDiskon, expectedDsikon);
		
		penjualanPage.valueDiskon();
	}
